...
Wd1      = GetWd1(dobs1,Nd);
%idx      = max(real(dobs))+1i*max(imag(dobs));
...
%            Jacob,JacobInv,JacobDet,Gauss3DW,Gauss,EPGauss,compress,symFlag,fileformat,Num_Comm,Q_Ex,Q_Ey,Q_Ez,Q_Hx,Q_Hy,Q_Hz,mpirun,xa,ya,za,minv_ind,Xtop,Ytop,Ztop,inv_flag);
    ...
    mtimes     = read_mtimes;
    save A_Seis A_seis mtimes
    %load A_seis_mtimes
    %A_seis = A_seis*max(dobs1);
    %mtimes = mtimes*max(dobs1);
    ...
    switch GramianType
        case 1
            ...
            % Calculate regularization parameter
             beta(n+1)    = GetJointPar_Ap(J,A_seis,Wd,Wd1,DG,qbeta,cbeta,n+1,p1,p2);
             WG_left      = beta(n+1)*DG;
...
%              WG_right     = beta(n+1)*SG(n+1);
  ...
  %  [dm,flag,relres,iter,resvec,lsvec] = lsqr_Implicit_A_MaxIter(A1_dense,A2_dense,Wm_left,WG_left,P,lsqr_tol,lsqr_maxit,p1,p2);
   ...
   %  sig_full(minv_ind) = Logm2m(m_full,mmin,mmax);
    ...
    kn = min(kn)
    kn1  =  Step_Linear_Search_log(Wd1,dobs1,m1Log,mtimes,dm2,m1max,m1min,Nmx,Nmy,Nmz)
