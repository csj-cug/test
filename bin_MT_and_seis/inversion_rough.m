...
%            Jacob,JacobInv,JacobDet,Gauss3DW,Gauss,EPGauss,compress,symFlag,fileformat,Num_Comm,Q_Ex,Q_Ey,Q_Ez,Q_Hx,Q_Hy,Q_Hz,mpirun,xa,ya,za,minv_ind,Xtop,Ytop,Ztop,inv_flag);
    ...
    p1 = (mmax-m).*(m-mmin) / (mmax-mmin);
    p2 = (m1max-m1).*(m1-m1min) / (m1max-m1min);
...
%              WG_right     = beta(n+1)*SG(n+1);
      ...
      A2_dense = Wd1*A_seis;
    P = -[Wd*real(rn);Wd*imag(rn);Wd1*rn1;WG_right;sqrt(apha(n+1))*(L)*(mLog-maprLog);sqrt(apha1(n+1))*(L)*(m1Log-m1aprLog)];
  % P = -[Wd*real(rn);Wd*imag(rn);Wd1*rn1;WG_right;sqrt(apha(n+1))*(L)*(mLog-maprLog);sqrt(apha1(n+1))*(L)*(m1Log-m1aprLog)];
  ...
  %  [dm,flag,relres,iter,resvec,lsvec] = lsqr_Implicit_A_MaxIter(A1_dense,A2_dense,Wm_left,WG_left,P,lsqr_tol,lsqr_maxit,p1,p2);
   ...
   % sig_full(minv_ind) = m_full;
    ...
    kn = min(kn)
    kn1  =  Step_Linear_Search_log(Wd1,dobs1,m1Log,mtimes,dm2,m1max,m1min)
...
%         end
%     end
      [gkn,gkn1]  = Step_Linear_Search_log_joint(mLog,dm1,mmin,mmax,m1Log,dm2,m1min,m1max)
      kn = (kn+gkn)/2;
      kn1 = (kn1+gkn1)/2;
%         if n>1 && misfit(n-1) < final_misfit
%             m1Log = m1Log+kn1*dm2;
