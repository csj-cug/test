function [x,flag,relres,iter,resvec,lsvec] = lsqr_Implicit_A_MaxIter(A1_dense,A2_dense,A2_sparse,WG_left,b,tol,maxit,p1,p2)
...
n = size(A1_dense,2) + size(A2_dense,2);
m = size(A1_dense,1) + size(A2_dense,2) + size(A2_sparse,1);
...
phibar = beta;
% v = A'*u;
v = Ax_Implicit(A1_dense,A2_dense,A2_sparse,WG_left,u,1,p1,p2);
if ~nKnown
...
for ii = 1 : maxit
   z = v;
    ...
    Az = Ax_Implicit(A1_dense,A2_dense,A2_sparse,WG_left,z,0,p1,p2);
    u = Az - alpha * u;
%     u = A*z - alpha * u;
    beta = norm(u);
    u = u / beta;
...
%         break
    ...
    normr = abs(s) * normr;
    resvec(ii+1) = normr;
    vt = Ax_Implicit(A1_dense,A2_dense,A2_sparse,WG_left,u,1,p1,p2);
%     vt = A'*u;
    v = vt - beta * v;
    alpha = norm(v);
    v = v / alpha;
...
function Ax = Ax_Implicit(A1_dense,A2_dense,A2_sparse,WG_left,x,trans,p1,p2)
m =  size(A1_dense,2);
nM = size(A1_dense,1);
nS = size(A2_dense,1);
n1 = size(A1_dense,1)+size(A2_dense,1)+size(WG_left,1);
x1 = x(1:m);
x2 = x(m+1:end);
% n2 = size(A2_sparse,1);
if trans
    %A1_densex  = [p1.*A1_dense'*x(1:nM);p2.*A2_dense'*x(nM+1:nM+nS)]+WG_left'*x(nM+nS+1:n1);
    A1_densex  = [A1_dense'*x(1:nM);A2_dense'*x(nM+1:nM+nS)]+WG_left'*x(nM+nS+1:n1);
   % A1_densex  = [p1.*A1_dense'*x(1:nM);p2.*A2_dense'*x(nM+1:nM+nS)]+WG_left'*x(nM+nS+1:n1);
    %Ax = A1_dense'*x(1:n1) + A2_sparse'*x((n1+1):end);
    Ax = A1_densex + A2_sparse'*x((n1+1):end);
else
   %  A1x = A1_dense*(p1.*x1);
   %  A2x = A2_dense*(p2.*x2);
    A1x = A1_dense*x1;
