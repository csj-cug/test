...
%addpath /public/home/caihongzhu/zhuhao/Joint/Joint_structured_Inv/input1
...
%m       = log(sigx(minv_ind));
%m       = sigx(minv_ind);
m      = m2Logm(log(sigx(minv_ind)),mmin,mmax);
m1      = vgrids;
mpre    = m;
mapr    = m;
m1apr   = m1;
...
%[Lx,Ly,Lz]      = MLSI3D_Rec_N_InvAnalytical(t_xyz_center_inv,t_xyz_center_inv,NumGradient);
for n=1:1+CN
    
      sigx(minv_ind) = exp(Logm2m(m,mmin,mmax));
      sigy(minv_ind) = exp(Logm2m(m,mmin,mmax));
      sigz(minv_ind) = exp(Logm2m(m,mmin,mmax));
...
%     p1 = spdiags((mmax-m).*(m-mmin) / (mmax-mmin),0,Nm,Nm);
    m11 = Logm2m(m,mmin,mmax);
    p1 = spdiags((mmax-m11).*(m11-mmin)  / (mmax-mmin),0,Nm,Nm);
...
%       x_1 = rand(Nm,1);
%       wj = Wd*(J*(p1.*x));
%       wj1 = Wd1*(A_seis*(p2.*x_1));
%       Ax = p1.*(J'*(Wd*wj));
%       Ax1 = p2.*(A_seis'*(Wd1*wj1));
    ...
    switch GramianType
        case 1
            ...
            % Calculate regularization parameter
             beta(n+1)    = GetJointPar_Ap(J,A_seis,Wd,Wd1,DG,qbeta,cbeta,n+1);
             WG_left      = beta(n+1)*DG;
             WG_right     = beta(n+1)*SG(n+1);
        case 2
%              SGx(n+1)                    = GetSGnew(mLog,m1Log,Lx);
%              SGy(n+1)                    = GetSGnew(mLog,m1Log,Ly);
%              SGz(n+1)                    = GetSGnew(mLog,m1Log,Lz);
%              DGx                         = GetDGnew1(mLog,m1Log,Lx,p1,p2);
%              DGy                         = GetDGnew1(mLog,m1Log,Ly,p1,p2);
%              DGz                         = GetDGnew1(mLog,m1Log,Lz,p1,p2);
% %             % Calculate regularization parameter
%              betax(n+1)                  = GetJointPar_Ap(J,A_seis,p1,p2,Wd,Wd1,DGx,qbeta,cbeta,n+1);
%              betay(n+1)                  = GetJointPar_Ap(J,A_seis,p1,p2,Wd,Wd1,DGy,qbeta,cbeta,n+1);
%              betaz(n+1)                  = GetJointPar_Ap(J,A_seis,p1,p2,Wd,Wd1,DGz,qbeta,cbeta,n+1);
...
%             % Calculate regularization parameter
             betax(n+1)                  = GetJointPar_Ap(J,A_seis,Wd,Wd1,DGx,qbeta,cbeta,n+1);
             betay(n+1)                  = GetJointPar_Ap(J,A_seis,Wd,Wd1,DGy,qbeta,cbeta,n+1);
             betaz(n+1)                  = GetJointPar_Ap(J,A_seis,Wd,Wd1,DGz,qbeta,cbeta,n+1);
             WG_left                     = [ betax(n+1)*DGx; betay(n+1)*DGy; betaz(n+1)*DGz ];
             WG_right                    = [ betax(n+1)*SGx(n+1); betay(n+1)*SGy(n+1); betaz(n+1)*SGz(n+1) ];

           
%              SG(n+1)      = GetSGnew(m,m1,I);
%              DG           = GetDGnew(m,m1,I);
%              % Calculate regularization parameter
%             % beta(n+1)    = GetJointPar_Ap(J,A_seis,p1,p2,Wd,Wd1,DG,qbeta,cbeta,n+1);
%             beta(n+1)    = GetJointPar_Ap(J,A_seis,Wd,Wd1,DG,qbeta,cbeta,n+1);
%              WG_left      = beta(n+1)*DG;
%              WG_right     = beta(n+1)*SG(n+1);
        case 3 
            DG            = GetDGnew(mLog,m1Log,I);
            DGx           = GetDGnew(mLog,m1Log,Lx);
            DGy           = GetDGnew(mLog,m1Log,Ly);
            DGz           = GetDGnew(mLog,m1Log,Lz);
            beta(n+1)     = GetJointPar_Ap(J,A_seis,p1,p2,Wd,Wd1,DG,qbeta,cbeta,n+1);
            betax(n+1)    = GetJointPar_Ap(J,A_seis,p1,p2,Wd,Wd1,DGx,qbeta,cbeta,n+1);
            betay(n+1)    = GetJointPar_Ap(J,A_seis,p1,p2,Wd,Wd1,DGy,qbeta,cbeta,n+1);
            betaz(n+1)    = GetJointPar_Ap(J,A_seis,p1,p2,Wd,Wd1,DGz,qbeta,cbeta,n+1);
            if n <= numMG
                SG(n+1)   = GetSGnew(mLog,m1Log,I);
                % Calculate regularization parameter
                WG_left   = beta(n+1)*DG;
                WG_right  = beta(n+1)*SG(n+1);
            else
          ...
          case 4
            DG            = GetDGnew1(m,m1,I,p1,p2);
            DGx           = GetDGnew1(m,m1,Lx,p1,p2);
            DGy           = GetDGnew1(m,m1,Ly,p1,p2);
            DGz           = GetDGnew1(m,m1,Lz,p1,p2);
            beta(n+1)     = GetJointPar_Ap(J,A_seis,p1,p2,Wd,Wd1,DG,qbeta,cbeta,n+1);
            betax(n+1)    = GetJointPar_Ap(J,A_seis,p1,p2,Wd,Wd1,DGx,qbeta,cbeta,n+1);
            betay(n+1)    = GetJointPar_Ap(J,A_seis,p1,p2,Wd,Wd1,DGy,qbeta,cbeta,n+1);
            betaz(n+1)    = GetJointPar_Ap(J,A_seis,p1,p2,Wd,Wd1,DGz,qbeta,cbeta,n+1);
            if n <= numMG
                ...
                SGx(n+1)  = GetSGnew(m,m1,Lx);
                SGy(n+1)  = GetSGnew(m,m1,Ly);
                SGz(n+1)  = GetSGnew(m,m1,Lz);
                % Calculate regularization parameter
                WG_left   = [betax(n+1)*DGx;betay(n+1)*DGy;betaz(n+1)*DGz];
                WG_right  = [ betax(n+1)*SGx(n+1); betay(n+1)*SGy(n+1); betaz(n+1)*SGz(n+1) ];
            end            
    ...
    A2_dense = Wd1*A_seis;
    P = -[Wd*real(rn);Wd*imag(rn);Wd1*rn1;WG_right;sqrt(apha(n+1))*(L)*(m-mapr);sqrt(apha1(n+1))*(L)*(m1Log-m1aprLog)];
  ...
  %  [dm,flag,relres,iter,resvec,lsvec] = lsqr_Implicit_A_MaxIter(A1_dense,A2_dense,Wm_left,WG_left,P,lsqr_tol,lsqr_maxit,p1,p2);
    ...
    %m_full   = mLog + dm1;
    sig_full = sigx;
    %sig_full(minv_ind) = exp(m_full);
   % sig_full(minv_ind) = m_full;
   %  sig_full(minv_ind) = Logm2m(m_full,mmin,mmax);
    sig_full(minv_ind) = exp(Logm2m(m_full,mmin,mmax));
...
%     if qbeta == 0                     %Separate Inv
%         if n>1 && misfit(n-1) < final_misfit
%             m1Log = m1Log+kn1*dm2;
%             %m     = m + kn*dm1;
%         elseif n>1 && misfit1(n-1) < final_misfit
%             mLog   = mLog +kn*dm1;
%         else
%             mLog   = mLog+kn*dm1;
%             m1Log  = m1Log+kn1*dm2;
...
%     else                              %Joint Inv
%         if GramianType<=2
%            [gkn,gkn1]  = Step_Linear_Search_log_joint(m,dm1,m1Log,dm2,m1max,m1min);
%            kn = (kn+gkn)/2;
%            kn1 = (kn1+gkn1)/2;
%            m     = mpre + kn*dm1;
%            m1Log  = m1Log + kn1*dm2;
%         else
%            [gxkn,gxkn1] = Step_Linear_Search_log_joint_G(m,dm1,mmax,mmin,m1Log,dm2,m1max,m1min,Lx);
%            [gykn,gykn1] = Step_Linear_Search_log_joint_G(m,dm1,mmax,mmin,m1Log,dm2,m1max,m1min,Ly);
%            [gzkn,gzkn1] = Step_Linear_Search_log_joint_G(m,dm1,mmax,mmin,m1Log,dm2,m1max,m1min,Lz);
...
%         end
%     end
     % [gkn,gkn1]  = Step_Linear_Search_log_joint(m,dm1,m1Log,dm2,m1min,m1max)
      [gkn,gkn1] = Step_Linear_Search_log_joint(m,dm1,mmin,mmax,m1Log,dm2,m1min,m1max);
      kn = (kn+gkn)/2;
      kn1 = (kn1+gkn1)/2;
%         if n>1 && misfit(n-1) < final_misfit
%             m1Log = m1Log+kn1*dm2;
...
%         end
   ...
   % m  = mpre + kn*dm1;
    %m  =  Logm2m(mLog,mmin,mmax);
    m1  =  Logm2m(m1Log,m1min,m1max);
