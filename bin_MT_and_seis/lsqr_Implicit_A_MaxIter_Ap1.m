function [x,flag,relres,iter,resvec,lsvec] = lsqr_Implicit_A_MaxIter_Ap(A1,A2,p1,p2,Wd1,Wd2,WG_left,A2_sparse,b,tol,maxit)
...
phibar = beta;
% v = A'*u;
v = Ax_Implicit(A1,A2,p1,p2,Wd1,Wd2,WG_left,A2_sparse,u,1);
if ~nKnown
...
for ii = 1 : maxit
   z = v;
    ...
    Az = Ax_Implicit(A1,A2,p1,p2,Wd1,Wd2,WG_left,A2_sparse,z,0);
    u = Az - alpha * u;
%     u = A*z - alpha * u;
    beta = norm(u);
    u = u / beta;
...
%         break
    ...
    normr = abs(s) * normr;
    resvec(ii+1) = normr;
    vt = Ax_Implicit(A1,A2,p1,p2,Wd1,Wd2,WG_left,A2_sparse,u,1);
%     vt = A'*u;
    v = vt - beta * v;
    alpha = norm(v);
    v = v / alpha;
...
function Ax = Ax_Implicit(A1,A2,p1,p2,Wd1,Wd2,WG_left,A2_sparse,x,trans)
m = size(A1,2);
nG = size(A1,1);
nM = size(A2,1);
n1 = size(A1,1)+size(A2,1)+size(WG_left,1);
% n2 = size(A2_sparse,1);
x1 = x(1:m);
x2 = x(m+1:end);
if trans
    x1 = x(1:nG);x2 = x(nG+1:nG+nM);x3 = x(nG+nM+1:n1); 
  %  A1_densex1 = p1.*(A1'*Wd1'*x1);
  %  A1_densex2 = p2.*(A2'*(Wd2'*x2));
    A1_densex1 = p1.*(A1'*x1);
    A1_densex2 = p2.*(A2'*x2);
    A1_densex3 = WG_left'*x3;
    A1_densex  = [A1_densex1;A1_densex2]+A1_densex3;
    Ax = A1_densex + A2_sparse'*x((n1+1):end);
else
   % A1x = [Wd1*(A1*(p1.*x1));Wd2*(A2*(p2.*x2));WG_left*x] ;
    A1x = [A1*(p1.*x1);A2*(p2.*x2);WG_left*x] ;
    A2x = A2_sparse*x;
    Ax  = [A1x;A2x];
end