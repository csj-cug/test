function [x,flag,relres,iter,resvec,lsvec] = lsqr_Implicit_A_MaxIter_Ap(A1,A2,Wd1,Wd2,WG_left,A2_sparse,b,tol,maxit)
...
phibar = beta;
% v = A'*u;
v = Ax_Implicit(A1,A2,Wd1,Wd2,WG_left,A2_sparse,u,1);
if ~nKnown
...
for ii = 1 : maxit
   z = v;
    ...
    Az = Ax_Implicit(A1,A2,Wd1,Wd2,WG_left,A2_sparse,z,0);
    u = Az - alpha * u;
%     u = A*z - alpha * u;
    beta = norm(u);
    u = u / beta;
...
%         break
    ...
    normr = abs(s) * normr;
    resvec(ii+1) = normr;
    vt = Ax_Implicit(A1,A2,Wd1,Wd2,WG_left,A2_sparse,u,1);
%     vt = A'*u;
    v = vt - beta * v;
    alpha = norm(v);
    v = v / alpha;
...
function Ax = Ax_Implicit(A1,A2,Wd1,Wd2,WG_left,A2_sparse,x,trans)
m = size(A1,2);
nG = size(A1,1);
nM = size(A2,1);
