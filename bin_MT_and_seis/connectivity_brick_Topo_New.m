function output = connectivity_brick_Topo_New(X,Y,Z,Xtop,Ytop,Ztop,BoundUp,BoundLow,ShiftZ,AspectChangeRange)
%% For this version, the Topography(Seafloor bathymetry) will be incorporated to the grid by shifting in vertical direction
%% BoundUp, BoundLow: two z surfaces that bound the topography, the z shift of these two surface is set to be 0. 
%% ShiftZ: The plane at which it will be shifted according to topography
...
%% AspectChangeRange: after distortion, the new dz (edge size in z direction) should be between AspectChangeRange(1) and AspectChangeRange(2) times of the original length

if length(AspectChangeRange) ~= 2
    error('AspectChangeRange should be a vector with two element');
else
    if AspectChangeRange(2)<=AspectChangeRange(1)
        error('The second element of AspectChangeRange should be larger than the first element');
    end
end
...
%%Shift
Zold   = Z;
ShiftRatio  = 1:-0.01:0.1;
indUp       = find(z==BoundUp);
indLow      = find(z==BoundLow);
indShift    = find(z==ShiftZ);

if isempty(indUp) || isempty(indLow) || isempty(indShift)
    error('The input shift plane or its up/low bound is not exist in the grid');
end



Ztop_interp = griddata(Xtop,Ytop,Ztop,X(:,:,1),Y(:,:,1));
Ztop_interp(isnan(Ztop_interp))=ShiftZ;
Z(:,:,indShift)=Ztop_interp;
Dshift = Ztop_interp - ShiftZ; %shiftamount
DshiftAbs = abs(Dshift);
maxshift  = max(DshiftAbs(:));





% for it = indShift-1:-1:indUp+1
%     DzOld = abs(unique(Zold(:,:,it)-Zold(:,:,it+1)));
%     
%     for ishift = 1:length(ShiftRatio)
%         Z(:,:,it) = Zold(:,:,it) + ShiftRatio(ishift)*Dshift;
%         Dz = min(abs(unique(Z(:,:,it)-Z(:,:,it+1))));
%         if Dz <= DzOld/AspectChangeTol;
%             ratio = ShiftRatio(ishift);
%             break;
%         end  
%     end
%     
...
for it = indShift-1:-1:indUp+1
    DzOld = abs(unique(Zold(:,:,it)-Zold(:,:,it+1)));
    Dz_min = zeros(size(ShiftRatio));
    Dz_max = zeros(size(ShiftRatio));
    for ishift = 1:length(ShiftRatio)
        Z(:,:,it) = Zold(:,:,it) + ShiftRatio(ishift)*Dshift;
        Dz_min(ishift) = min(abs(unique(Z(:,:,it)-Z(:,:,it+1))));
        Dz_max(ishift) = max(abs(unique(Z(:,:,it)-Z(:,:,it+1))));
    end
    
    ind1 = find(Dz_min >= DzOld*AspectChangeRange(1) & Dz_max <= DzOld*AspectChangeRange(2));
    ratio = ShiftRatio(ind1(end));
    
    if it < indShift-1
        if ratio > ratiopre
            warning('shiftratio should decrease');
            ratio = 0.99*ratiopre;
        end
    end
    
    
    Z(:,:,it) = Zold(:,:,it) + ratio*Dshift;
    ratiopre = ratio; 
end
...
%     for ishift = 1:length(ShiftRatio)
%         Z(:,:,it) = Zold(:,:,it) + ShiftRatio(ishift)*Dshift;
%         Dz_min = min(abs(unique(Z(:,:,it)-Z(:,:,it-1))));
%         Dz_max = max(abs(unique(Z(:,:,it)-Z(:,:,it-1))));
%         if Dz_min <= DzOld/AspectChangeTol || Dz_max >= DzOld*AspectChangeTol;
%             ratio = ShiftRatio(ishift);
%             break;
%         end  
%     end
...
nedy = nx*(ny-1)*nz;
nedz = nx*ny*(nz-1);
ned  = nedx + nedy + nedz; %total number of edges
...
ed_tmp      = [edx;edy;edz];
[ed Ind]     = sortrows(ed_tmp,[1 2]);

ed_tmp_comp = ed_tmp(:,1)+1i*ed_tmp(:,2);
ed_comp     = ed(:,1)+1i*ed(:,2);

[tmp,~,Ind1] = intersect(ed_tmp_comp,ed_comp,'stable');  %%Here ed_tmp=ed(Ind1);
...
xyz1            = p(ed(:,1),:); %coordinate for the first nodes of each edge
xyz2            = p(ed(:,2),:);
...
if min(diff(Z,1,3)) <=0
    error('Mesh Crashed. Z mesh should increase even after shifting');
end
...
output.Ind1         = Ind1;
output.Ztop_interp  = Ztop_interp;
output.Z_new        = Z;
